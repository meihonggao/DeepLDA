...
def train(model,args,dataset):
    ...
    for epoch in range(args.epochs):
        ...
        auc_train,aupr_train,F1_train,mcc_train = metrics(output[dataset['idx_train']], dataset['labels'][dataset['idx_train']])
        #total_auc.append(auc_train)
        #total_aupr.append(aupr_train)
        #total_f1.append(F1_train)
        #total_mcc.append(mcc_train)
        loss_train.backward()
        optimizer.step()
...
def compute_test(model,dataset):
    ...
    auc_train,aupr_train,F1_train,mcc_train = metrics(output[dataset['idx_train']], dataset['labels'][dataset['idx_train']])
    auc_val,aupr_val,F1_val,mcc_val = metrics(output[dataset['idx_val']], dataset['labels'][dataset['idx_val']])
    auc_test,aupr_test,F1_test,mcc_test = metrics(output[dataset['idx_test']], dataset['labels'][dataset['idx_test']])
